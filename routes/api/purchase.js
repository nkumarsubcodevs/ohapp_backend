  ...
  goalObject.getPartnerData(user_id, function (err, partnerMappingData) {
    if (err) {
      ...
      if (partnerMappingData) {
        ...
        SubscripationObject.saveSubscripation(data, function (
          err,
          responseData
        ) {
          if (err) {
            ...
            if (responseData) {
              userSerObject.UpdateSubscripation(data, function (
                err,
                UpdateData
              ) {
                if (err) {
                  ...
                  if (UpdateData) {
                    userSerObject.getUserById(user_id, function (
                      err,
                      UserData
                    ) {
                      if (err) {
                                        ...
                                        if (err) {
                                          ...
                                          if (PartnersData) {
                                            res.send({
                                              ...
                                              partnerStage: PartnersData.stage,
                                            });
                                          } else {
                                            res.send({
                                              status: 400,
  ...
  userSerObject.getUserById(user_id, function (err, UserData) {
    if (err) {
      ...
      if (UserData) {
        if (UserData.stage >= 4) {
          if (UserData.receipt) {
            SubscripationObject.VerifyReceipt(UserData, function (
              err,
              VerifyData
            ) {
              if (err) {
                ...
                if (VerifyData) {
                  if (VerifyData.data.expire_at) {
                    ...
                    if (currentDate.getTime() < expiryDate.getTime()) {
                      ...
                      userSerObject.getPartnerById(user_id, function (
                        err,
                        PatnerData
                      ) {
                        if (err) {
            ...
            userSerObject.getPartnerById(user_id, function (err, PatnerData) {
              if (err) {
                ...
                if (PatnerData) {
                  if (PatnerData.receipt) {
                    SubscripationObject.VerifyReceipt(PatnerData, function (
                      err,
                      VerifyData
                    ) {
                      if (err) {
...
router.get("/subscripation", verifyToken, function (req, res) {
  let user_id = jwt.decode(req.headers["x-access-token"]).id;
  SubscripationObject.getSubscripation(user_id, function (
    err,
    SubscripationData
  ) {
    if (err) {
  ...
  userSerObject.getPartnerById(user_id, function (err, partnerData) {
    if (err) {
      ...
      if (partnerData) {
        ...
        userSerObject.updateBothPartnerStageByFaild(updateStage, function (
          err,
          updatedData
        ) {
          if (err) {
