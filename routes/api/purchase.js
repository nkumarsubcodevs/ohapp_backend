							...
							if (err) {
								...
								if (VerifyData) {
									if (VerifyData.data.expire_at) {
										let expiryDate = VerifyData.data.expire_at;
										// if (UserData.device_name === 'ios') {
										// 	let exDate = VerifyData.data.expire_at.replace(' Etc/GMT', '');
										// 	expiryDate1 = new Date(exDate).toISOString();
										// 	console.log(VerifyData.data.expire_at)
										// 	console.log(expiryDate1.getTime())
										// 	expiryDate = new Date(parseInt(VerifyData.data.expire_at))
																		...
																		if (VerifiesData) {
																			if (VerifiesData.data.expire_at) {
																				...
																				if (PatnerData.device_name === 'ios') {
																					...
																					PatnerData.device_name === 'android'
																				) {
																					expiryDate = VerifiesData.data.expire_at;
																				}
										...
										SubscripationObject.VerifyReceipt(PatnerData, function (err, VerifyData) {
											...
											if (err) {
												...
												if (VerifyData) {
													if (VerifyData.data.expire_at) {
														let expiryDate;
														expiryDate = VerifyData.data.expire_at;
														let currentDate = current_datetime.format(
															new Date(),
															'YYYY-MM-DD HH:mm:ss',
															true
...
router.get('/subscripation', verifyToken, function (req, res) {
					...
					if (UserData.receipt) {
						SubscripationObject.VerifyReceipt(UserData, function (err, VerifyData) {
							if (err) {
								...
								if (VerifyData) {
									if (VerifyData.data.expire_at) {
										let expiryDate = VerifyData.data.canceled_at || VerifyData.data.expire_at;
										let currentDate = current_datetime.format(
											new Date(),
											'YYYY-MM-DD HH:mm:ss',
											true
										...
										if (currentDate.getTime() < expiryDate.getTime()) {
											...
											SubscripationObject.updateStage(updateStage, function (err, updatedData) {
												res.send({
													status: 400,
													message: 'Plan is not active!!!',
													payment: 0,
	...
	// 		console.log(err)
	// 	} else {
	// 		console.log("SubscripationData", SubscripationData)
