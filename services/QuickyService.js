...
class QuickyService {
	...
	// save all notification before get the notification
	async saveQuickybeforenotification(quikceydata, callback) {
		let saverecords = new QuickyObject({
			user_id: quikceydata.user_id,
			partner_mapping_id: quikceydata.partner_mapping_id,
			when: "afterselectgoal",
			where: "afterselectgoal",
			create_time: current_datetime.format(now, 'YYYY-MM-DD hh:mm:ss'),
			update_time: current_datetime.format(now, 'YYYY-MM-DD hh:mm:ss')
		})
		callback(null, await saverecords.save());
	}
	...
	// update partner data fot quikey
	...
	// add contribution using quikie id
