...
const untrackObject = require("../models/untrack");
...
class GoalService {
  ...
  //save untrack data
  async SaveUntrack(untrackdata, callback) {
    const now = new Date();
    let untrack = new untrackObject({
      user_id: untrackdata.user_id,
      whoinitiative: untrackdata.whoinitiative,
      createddate: current_datetime.format(now, "YYYY-MM-DD hh:mm:ss"),
      create_time: current_datetime.format(now, "YYYY-MM-DD hh:mm:ss"),
      update_time: current_datetime.format(now, "YYYY-MM-DD hh:mm:ss")
    })
    untrack.save()
  ...
  //get untrackdata 
  async getUntrackdata(data, parner_data, callback) {
    //var partner_id = 0;
    const TODAY_START = new Date().setHours(0, 0, 0, 0);
    const NOW = current_datetime.format(new Date(), "YYYY-MM-DD hh:mm:ss", true);
    const partnerResponse = await untrackObject.findAll({
      where: {
        [Op.or]: [

          {
            user_id: {
              [Op.eq]: parner_data.partner_one_id,
            },
          },
          {
            user_id: {
              [Op.eq]: parner_data.partner_two_id,
            },
          },
        ],
  ...
  async updateQuestion(title, descripation, option, id,) {
    const now = new Date();
    await goalSettingsObject.update({ question_descripation: descripation, question_title: title, iscustom: option, update_time: now }, { where: { id: id } })
  }
}
